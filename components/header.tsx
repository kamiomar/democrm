...
export function Header({ onMenuClick }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 h-16 px-4 sm:px-6 border-b bg-zinc-950/90 backdrop-blur-sm border-zinc-800 dark:bg-zinc-950/90 dark:border-zinc-800 light:bg-white/90 light:border-slate-200">

      {/* Hamburger — mobile only */}
      <button
        onClick={onMenuClick}
        className="lg:hidden flex items-center justify-center w-8 h-8 rounded-lg text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-100 dark:hover:bg-zinc-800 light:text-slate-500 light:hover:text-slate-900 light:hover:bg-slate-100 transition-colors flex-shrink-0"
      ...
      <div className="flex-1 min-w-0">
        <h1 className="text-base font-semibold text-zinc-100 dark:text-zinc-100 light:text-slate-900 leading-tight truncate">{title}</h1>
      ...
      <div className="hidden md:flex items-center gap-2 px-3 h-8 w-56 rounded-lg bg-zinc-900 border border-zinc-800 dark:bg-zinc-900 dark:border-zinc-800 light:bg-slate-100 light:border-slate-200">
        ...
        />
        <kbd className="text-[10px] text-zinc-600 dark:text-zinc-600 light:text-slate-400 font-mono hidden lg:inline">⌘K</kbd>
      </div>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <ThemeToggle />
        <div className="ml-1 w-7 h-7 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-xs font-semibold text-white cursor-pointer flex-shrink-0">
          AC
        </div>
      </div>
    </header>
